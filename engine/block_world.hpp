...
namespace engine {
    ...
    class LimitedBlockWorld {
        ...
        [[nodiscard]] bool isInBounds(const Vector3& pos) const {
            const Vector3 absPos = pos.abs();
            return absPos.x < quadrantSize.x && absPos.y < quadrantSize.y && absPos.z < quadrantSize.z;
        }
    public:
        LimitedBlockWorld(const size_t x, const size_t y, const size_t z) : quadrantSize({ x, y, z }), mapSize(x * 2, y * 2, z * 2) {
            const size_t size = mapSize.z * mapSize.y * mapSize.x;
